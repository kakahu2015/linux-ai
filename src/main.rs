...
impl LinuxCommandAssistant {
    ...
    fn new(config: Config) -> Result<Self> {
        ...
        let excluded_commands = vec![
            "top".to_string(), "htop".to_string(), "tail -f".to_string(),
            "make".to_string(), "gcc".to_string(), "apt upgrade".to_string(),
            // 添加其他需要排除的命令
        ];
...
///////////////////////////////
 // 添加一个辅助方法来检查命令是否被排除
    fn is_excluded_command(&self, command: &str) -> bool {
        self.excluded_commands.iter().any(|excluded| command.starts_with(excluded))
    }
/////////////////////////////////////////////
fn execute_command(&mut self, command: &str) -> Result<String> {
    ...
    if !self.is_excluded_command(command) {
        ...
        // 对于被排除的命令，只记录执行的事实
        self.add_to_recent_interactions(format!("Executed excluded command: {}", command));
    }
